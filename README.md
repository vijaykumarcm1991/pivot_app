...
| Commit    | Description                                                                                       |
| --------- | ------------------------------------------------------------------------------------------------- |
| (latest)  | `9f4524a` — **fix**: auto-scroll to the AG Grid on narrow viewports after clicking Compute Pivot, so the result is always visible without manual scrolling. `bb98627` — **fix**: cache-bust the pivot page JS files (`?v=2`) so a stale browser doesn't keep loading the old `pivot.js` / `pivot-grid.js` after a deploy. |
