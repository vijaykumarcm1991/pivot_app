...
| Commit    | Description                                                                                       |
| --------- | ------------------------------------------------------------------------------------------------- |
| (latest)  | `feat(email+users)`: User Directory typeahead in the email composer. The user uploads `users.json` to the project root (mounted read-only at `/app/users.json` inside the container). The email composer's To / CC / BCC fields now show a real typeahead on every keystroke — directory matches (name + email + department + job title) are shown first; recent recipients are a fallback when no directory match is found. New `app/services/user_directory.py` (caches the parsed list, auto-reloads on mtime change, exposes `search` / `reload` / `status`). New endpoints: `GET /api/users/suggest`, `GET /api/users/recent`, `GET /api/users/status`, `POST /api/users/reload`. New "User Directory" card on the Settings page shows total / enabled counts + last-loaded time (IST) + a manual "Reload now" button. The composer is now click-free for typeahead (Bootstrap's toggle is replaced with a manual `show` class on the dropdown, controlled by `email-manager.js`). |
